...
Реалізація — в [[file:main-tool-items.org][~main-tool-items.org~]] (тангл таргета ~src/tools/items.rs~). Залежить від власного форку
~sdk-internal~ (див. [[file:config.org][~config.org~]]) — на апстрімі жодна з чотирьох дій не спрацювала б.
